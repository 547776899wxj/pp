<template>
	<view  id="containerId">
		...
		<!-- 搜索框 -->
		<view class="flex-center pl32 pr32 invite-box">
			<image src="/static/img/icon/goods-icon@2x.png" mode="" class="goods-icon mr26"></image>
			<view class="search-box dflex ac">
				<image @click="refreshGoods()" src="/static/img/icon/search-icon@2x.png" class="search-icon ml32 mr20">
				</image>
				<input type="text" v-model="queryObj.goodsTitle" placeholder="" class="search-input"
					placeholder-class="fc-939" />
			</view>
		</view>
		<!-- 分类 -->
		<view>
			<view class="goods-navlist dflex ac pl16 pr160">
				<view v-for="(item,index) in category" class="dflex fs-0">
					<view class="goods-nav" @click="categoryClick(item.goodsCategoryId)"
						:class="queryObj.goodsCategoryId == item.goodsCategoryId?'cur':''">{{item.goodsCategoryName}}
					...
					<view class="goods-list" @click.stop="goToGoodsDetail(item.goodsId)"
						...
						<view class="flex-between fdc wp100 goods-info">
							...
							<view class="flex-between ai-fe">
								...
								<view v-if="item.goodsStock <= 0" class="small-btn m-bg-pp-unclick fc-f lh-60 tc">已售罄
								</view>
								<view v-else class="small-btn bg-pp fc-f lh-60 tc" @click.stop="addOrder(item.goodsId)">
									立即购买</view>
							</view>
						</view>
					</view>
...
<script>
	...
	export default {
		...
		onLoad(e) {
			console.log("goods e:" + JSON.stringify(e))
			this.option = e
		},
		onShow() {
			uni.$utils.setTabBarOrderNum()
			const goodsCategoryId = uni.getStorageSync('goodsCategoryId');
			if (goodsCategoryId) {
				this.queryObj.goodsCategoryId = goodsCategoryId;
				this.reFreshPage();
				try {
					uni.removeStorageSync('goodsCategoryId');
				} catch (e) {
					// error
				}
			}else{
