		...
		<view class="container">
			<view class="order-info" v-for="item in list.rows" @click="gotoOrderDetail" :data-order-id="item.orderId">
				<view class="flex-between ac mb20">
					<view class="fs-26 lh-36 fc-303 fw-b">{{item.lockBreakTime}}</view>
				...
				<view class="dflex jc-end ai-fe mt30">
					<view class="fs-20 lh-28 fc-303">违约金额：</view>
	...
	export default {
		...
		methods: {
			onFetchData(){
				...
				uni.$api.getBalanceBreakList(this.queryParam).then(res =>{
					console.log('res',res)
