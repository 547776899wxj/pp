	...
	export default {
		...
		methods: {
			...
			getAllAddress () {
				uni.$api.getAllAddress().then(res=>{
					if (res.success) {
						...
						if(!this.addressId){
							this.orderAddress = this.addressList.find(e=>e.addressDefaultStatus===1)||this.addressList[0]||{}
			...
			chooseAddress (e) {
				...
				this.orderAddress = this.addressList.find(b=>b.addressId===e.addressId)
