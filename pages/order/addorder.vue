	...
	<view>
		...
		<view class="container">
			...
			<view class="order-info">
				<!-- 订单详情列表 -->
				<view class="goods-info" v-if="details &&details.length > 0" v-for="(item,index) in details" :Key="index">
					...
					<view class="wp100 goods-msg flex-between fdc">
						<view class="dflex fdc jc-fs">
							...
							<view class="fs-20 lh-28 fc-939" v-if="item.orderGoodsSubTitle">{{item.orderGoodsSubTitle}}</view>
						</view>
				...
				<view class="flex-between ai-fs mt24" v-if="order.orderOtherMoney" >
					<view class="dflex ac">
						<view class="fs-24 lh-24 fc-303">运费</view>
					</view>
					<view class="dflex jc-end ai-fe">
						<view class="fs-24 fc-303">￥</view>
						<view class="fs-32 lh-32 fc-303">{{getMoneyForYuan(order.orderOtherMoney)}}</view>
						<view class="fs-20 fc-303">{{getMoneyForFen(order.orderOtherMoney)}}</view>					
					</view>
				</view>
				
				<view class="dflex jc-end ai-fe mt24">
					<!-- 目前一个商品只能买一件 -->
					<view class="fs-20 lh-28 fc-939 mr16">共{{details.length}}件</view>
					<view class="fs-20 lh-28 fc-303">小计：</view>
					<view class="fs-24 fc-e31">￥</view>
					<view class="fs-32 lh-32 fc-e31">{{getMoneyForYuan(order.orderMoney)}}</view>
					<view class="fs-20 fc-e31">{{getMoneyForFen(order.orderMoney)}}</view>
				</view>
				
				
				
				<view class="flex-between ai-fs mt24" v-if="discountMoneys &&discountMoneys.length > 0" v-for="(item,index) in discountMoneys" :Key="index">
					<view class="dflex ac">
						<view class="discount-icon flex-center mr8">{{getDiscountTagName(item)}}</view>
						<view class="fs-24 lh-24 fc-303">{{getDiscountTypeName(item)}}</view>
					</view>
					<view class="dflex jc-end ai-fe">
						<view class="fs-24 fc-e31">-￥</view>
						<view class="fs-32 lh-32 fc-e31">{{getMoneyForYuan(item.orderMoneyValue)}}</view>
						<view class="fs-20 fc-e31">{{getMoneyForFen(item.orderMoneyValue)}}</view>
					</view>
				</view>
			</view>
