	...
	export default {
		...
		methods: {
			...
			getAuctionLotStock() {
				...
				uni.$api.getAuctionLotStock(this.queryObj).then(res => {
					const obj = res.data || {}
					for (let key in obj) {
			...
			getAuction() {
				this.getLots()
				uni.$api.getAuction(this.queryObj).then(res => {
			...
			countDown() {
				...
				const beginTime = new Date(this.auction.auctionBeginTime.replace(/\-/g, '/')).getTime()
				const vipTime = beginTime - 60000
				const endTime = new Date(this.auction.auctionEndTime.replace(/\-/g, '/')).getTime()
				const _this = this
				if (this.timerId) {
					clearInterval(this.timerId)
				...
				this.timerId = setInterval(() => {
					...
					if (_this.nowTime > beginTime && _this.nowTime <= endTime) {
						_this.countDownStr = '距结束' + uni.$utils.date.countDown(_this.nowTime, endTime)
						_this.auction.auctionStatus = 1
						_this.$forceUpdate()
					} else if (_this.nowTime <= beginTime) {
						_this.countDownStr = '距开始' + uni.$utils.date.countDown(_this.nowTime, beginTime)
						if (_this.nowTime >= vipTime && _this.vipCoupon) {
							_this.auction.auctionStatus = 1
							_this.$forceUpdate()
						}
			...
			getLots() {
				...
				uni.$api.getLots(this.queryObj).then(res => {
					...
					this.loading = false
					this.$nextTick(() => {
						// console.log('rendertime=====>'+(new Date().getTime()-time)+'ms')
					})
					const _this = this
					setTimeout(() => {
